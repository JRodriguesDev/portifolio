import type { Project } from '@/types/project';

export const projects: Project[] = [
   {
      title: 'E-commerce Full Stack',
      shortDescription:
         'Um projeto de estudo prático criado para aplicar conceitos de desenvolvimento web na criação de um e-commerce Full Stack. O foco do repositório é documentar meu aprendizado ao estruturar integrações de pagamentos, gerenciar o estado e o cache da aplicação e implementar uma arquitetura de dados segura com TypeScript, Next.js e PostgreSQL.',
      images: ['/images/Ecommerce1.webp'],
      github: 'https://github.com/JRodriguesDev/Ecommerce',
      readme: 'https://github.com/JRodriguesDev/Ecommerce#readme',
      featured: true,
      status: 'Concluído',
      highlights: [
         'Stripe Custom Checkout',
         'Sistema de Assinaturas',
         'Auth.js + 2FA',
         'PostgreSQL + Prisma',
         'Docker',
         'Webhooks',
      ],
      technologies: [
         'Next.js',
         'TypeScript',
         'Tailwind CSS',
         'Prisma',
         'Docker',
         'Auth.js',
         'Zustand',
         'Stripe',
         'Zod',
         'Node.js',
         'React',
         'Shadcn/UI',
         'PostgreSQL',
         'Resend',
      ],
      learningFocus: [
         'Arquitetura Full Stack',
         'Integração com Stripe',
         'Server Actions',
         'Autenticação',
         'Webhooks',
      ],
   },
   {
      title: 'API E-commerce',
      shortDescription:
         'Este projeto é o resultado de um estudo sobre o desenvolvimento de ecossistemas de e-commerce. O objetivo principal foi construir um backend sólido e organizado, focado na gestão de pagamentos e assinaturas, aplicando boas práticas de arquitetura e segurança.',
      images: ['/images/apiBackend.webp'],
      github: 'https://github.com/JRodriguesDev/api_e-commerce',
      readme: 'https://github.com/JRodriguesDev/api_e-commerce#readme',
      featured: false,
      status: 'Concluido',
      highlights: [
         'Integração com Stripe',
         'Sistema de Assinaturas',
         'JWT + Cookies',
         'RBAC',
         'Redis Cache',
         'Swagger',
      ],
      technologies: [
         'Fastify',
         'TypeScript',
         'Prisma',
         'PostgreSQL',
         'Redis',
         'Stripe',
         'JWT',
         'Zod',
         'Swagger',
         'Docker',
         'Bcrypt',
      ],
      learningFocus: [
         'Arquitetura Backend',
         'Integração com Stripe',
         'Autenticação',
         'Controle de Permissões (RBAC)',
         'Cache com Redis',
         'Documentação com Swagger',
      ],
   },
   {
      title: 'Task Flow',
      shortDescription:
         'Aplicação Full Stack de gerenciamento de tarefas focada em produtividade e planejamento visual. Possui métricas em tempo real, pré-filtros ágeis, visualização em calendário e integração com a Google Calendar API para sincronização nativa de eventos.',
      images: ['/images/taskflow.webp'],
      github: 'https://github.com/JRodriguesDev/TaskFlow',
      readme: 'https://github.com/JRodriguesDev/TaskFlow#readme',
      featured: true,
      status: 'Concluido',
      highlights: [
         'Integração Google Calendar API',
         'Auth.js (OAuth Google + Credentials)',
         'Visualização em Calendário',
         'Métricas de Progresso Diário',
         'Ambiente Dockerizado',
         'UI Animada com Motion',
      ],
      technologies: [
         'Next.js',
         'React',
         'TypeScript',
         'Tailwind CSS',
         'Shadcn/UI',
         'Prisma',
         'PostgreSQL',
         'Auth.js',
         'Google APIs',
         'Docker',
         'Motion',
         'Zod',
      ],
      learningFocus: [
         'Integração com APIs de Terceiros (Google Calendar)',
         'Gerenciamento de Tokens OAuth e Sessões',
         'Visualização e Manipulação Temporal de Dados',
         'Containerização Multi-stage com Docker',
         'UX/UI Responsiva e Animações de Interface',
      ],
   },
];
